...
public:
    int countCells(vector<vector<char>>& grid, string pattern) {
        int m = grid.size();
        int n = grid[0].size();
        // 1. Create a combine string hor and ver
        ...
        // 2. Create LPS array for KMP to use
        int pLen = pattern.size();
        lps.resize(pLen, 0);
        countLPS(pattern);

        // 3. Run KMP on hor first to find the grid what can form pattern
        vector<int> horKMP(m*n, 0);
        int done = -1; // The index before "done" already change on grid to avoid duplicate trace when pattern is long
        for(int i=0; i<m*n; ++i){
            int len = i == 0? 0 : horKMP[i-1];
            if(len == pLen) len = lps.back();
            while(len > 0 && pattern[len] != hor[i]){
                len = lps[len-1];
            }
            if(pattern[len] == hor[i]) ++len;
            horKMP[i] = len;
            // mark on grid
            if(len == pLen){
                // put i-pLen+1 ~ i
        ...
        for(int i=0; i<m*n; ++i){
            ...
            if(pattern[len] == ver[i]) ++len;
            verKMP[i] = len;
            // check if the position also mark by hor
            if(len == pLen){
                int start = max(done+1, i-pLen+1);
                for(; start <= i; ++start){
                    int x = start / m;
