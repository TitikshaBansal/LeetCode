...
public:
    string numberToWords(int num) {
        ...
        for (int i = 0; i < 3; ++i) {
            if (num > 0 && num % 1000 > 0)  result = numberToWordsHelper(num % 1000) + bigString[i] + " " + result;
            num /= 1000;
        }
    ...
    string numberToWordsHelper(int num) {
        ...
        string result = "";
        if (num > 99) result += digitString[num / 100] + " Hundred ";
        num %= 100;
        if (num < 20 && num > 9) result += teenString[num - 10] + " ";
        else {
            if (num >= 20) result += tenString[num / 10] + " ";
            num %= 10;
            if (num > 0) result += digitString[num] + " ";
        }
        return result;
    }
};
